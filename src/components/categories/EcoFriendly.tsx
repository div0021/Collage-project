...
const EcoFriendly = () => {
  ...
  const handleFilterClick = () => {
    if (window.innerWidth <= 960) {
      dispatch(onFilterOpen({category:"Ecofriendly"}));
    } else {
      setShowFilter((pre) => !pre);
    }
  };

  const [ecoFriendlyProducts, setEcoFriendlyProducts] = useState<ProductType[]>([]);
  ...
  useEffect(()=>{
    if(ecoFriendlyProducts && ecoFriendlyProducts.length>0){

      let result:ProductType[] = ecoFriendlyProducts

      // price filter
      if(priceOptions){
        result = ecoFriendlyProducts.filter(product=>{
  ...
  },[ecoFriendlyProducts,categoryOptions,priceOptions,selectValue])


  const subCategories = useAppSelector(selectSubCategories);

  useEffect(() => {
    const getEcoFriendlyProduct = async () => {
      try {
        const url = import.meta.env.VITE_SERVER_URL;

        const response = await axios.get(`${url}/api/products/category/6619cbc61d8076ce9cb27fca`);


        setEcoFriendlyProducts(response.data.products);
      } catch (error) {
        console.log("GETOP", error);
      }
    };
    getEcoFriendlyProduct();
  }, []);
        ...
        <div className="flex flex-col items-center justify-center overflow-x-hidden">
          <div className="mt-10 sm:mt-32 mb-10 max-w-7xl px-5 sm:px-10 2xl:px-0">
            ...
            <div className="space-y-3 lg:space-y-0 lg:px-0 mt-10 lg:flex lg:justify-between lg:items-center">
              <h2 className="text-5xl font-bold tracking-wide">EcoFriendly</h2>
              <p className="lg:w-2/3 text-gray-700">
                {ecoFriendlyProducts && ecoFriendlyProducts.length>0 && ecoFriendlyProducts[0].category.description}
              </p>
            </div>
            ...
            <div className="flex mt-5">
              ...
              >
                {subCategories.length > 0 ? (<FilterSection categoryTitle="Ecofriendly" filterHeading="Sub Categories" data={subCategories} />):(
                  <div className="w-full h-52 flex justify-center items-center">
                     <CircularLoader />
                  </div>
                )}

                <FilterCheckboxSection categoryTitle="Ecofriendly" filterHeading="Price" data={price}/>
                

                <div className="w-full flex justify-start items-center">

                  <Button icon={GrPowerReset} label="Reset" className="w-52 hover:text-white" onClick={()=>{
                    dispatch(resetFilterOptions({category:"Ecofriendly"}))
                  }} />
